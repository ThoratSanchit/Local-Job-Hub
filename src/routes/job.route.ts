...
import { createJobSchema, updateJobSchema, searchJobSchema, getJobsSchema } from '../schemas/job.schema';
...
const jobRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.post('/search', { schema: searchJobSchema }, JobController.searchJobs);
  fastify.get('/recent-searches', JobController.getRecentSearches);
